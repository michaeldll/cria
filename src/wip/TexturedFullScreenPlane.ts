...
export default class TexturedPlane extends Mesh {
	...
	constructor(renderer: Renderer, geometry: Geometry, shader: string) {
		super(renderer, geometry, shader);
	}
	...
	protected initAttributeBuffers(): void {
		const { positions, indices, uvs } = this.geometry;
	...
	protected getTexture(): [GPUTexture, GPUSampler] {
		...
		this.renderer.device.queue.writeTexture({ texture }, new Uint8Array([255, 255, 128, 255, 128, 255, 255, 255, 255, 128, 255, 255, 255, 128, 128, 255]), { bytesPerRow: 8, rowsPerImage: 2 }, { width: 2, height: 2 });
	...
	protected async getImageTexture(url: string): Promise<[GPUTexture, GPUSampler]> {
		return new Promise(async (resolve, reject) => {
			...
			this.renderer.device.queue.copyExternalImageToTexture({ source, flipY: true }, { texture }, { width: source.width, height: source.height });
