...
export default class Cube extends Mesh {
	...
	protected initAttributeBuffers = (): void => {
		const { positions, indices, normals, uvs } = this.geometry;
	...
	protected initTexture = (): void => {
		...
		this.device.queue.writeTexture({ texture: this.texture }, new Uint8Array([255, 255, 128, 255, 128, 255, 255, 255, 255, 128, 255, 255, 255, 128, 128, 255]), { bytesPerRow: 8, rowsPerImage: 2 }, { width: 2, height: 2 });
	};
	...
	public render = (camera: Camera, time: number): void => {
		...
		this.renderer.passEncoder.drawIndexed(this.geometry.indices.length);
